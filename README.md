...
ScriptWhat it doesChanges anything?3_inventory_agents.shAudits installed AI agents, config directories, credential files, MCP connectionsNo — read-only2_check_port_18789.shChecks for OpenClaw's default port, prints firewall commandsNo — read-only1_tighten_agent_permissions.shTightens directory and file permissionsOnly with --apply
Recommended Order
